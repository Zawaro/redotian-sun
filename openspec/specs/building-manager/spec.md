...
`can_place(building_type, origin_cell)` SHALL check all foundation cells for: map bounds, play area bounds, cell availability (not occupied by building, bib, resource, entity), and terrain height variation (max - min ≤ `HEIGHT_STEP`).

#### Scenario: Valid placement
- **WHEN** all foundation cells are free, in bounds, and height variation is within limit
...
- **THEN** only bounds check is enforced, cell checks are skipped

### Requirement: Building placement
`place_building(building_type, origin_cell)` SHALL validate placement, deduct cost (unless `_skip_next_deduction` is set), create the entity via EntityFactory, register cells in SpatialHash, register building in PrerequisiteSystem, and emit `building_placed`.
...
- **THEN** bib cells are registered separately in SpatialHash
...
`_is_cell_free(cell)` SHALL return false if the cell is: a building cell, blocked, has an entity with MovementController, a bib cell, has a resource, or terrain type is not "" or "clear".
