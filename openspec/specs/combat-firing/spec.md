...
When firing, CombatComponent SHALL call `target.get_node("HealthComponent").take_damage(weapon.damage, weapon.warhead)` directly (hitscan). No projectile node is spawned.

#### Scenario: Damage applied
- **WHEN** CombatComponent fires at a target within range with cooldown elapsed
- **THEN** the target's HealthComponent SHALL receive `take_damage(damage, warhead)` call
