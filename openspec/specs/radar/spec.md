...
Track which players have radar capability at runtime: a registry of `RadarComponent` nodes in the scene tree, aggregated into per-player availability that radar-dependent UI (the gameplay minimap) consumes. Availability flips drive a change signal so consumers react to power state, destruction, and sale, and a debug override lets debug builds exercise radar-dependent UI without owning a radar structure.
