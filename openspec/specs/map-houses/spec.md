...
The system SHALL provide a canonical house id vocabulary module (`Houses.gd`) whose ids are `GDI`, `Nod`, `Neutral`, and `Special`, matching the ownership strings already used by `EntityData.owner` and the editor/sidebar faction colors. Houses are factions (the TS rules-side `[Houses]` list); player slots are a separate axis (start locations / waypoints 0-7) and SHALL NOT be modeled as houses. The module SHALL expose `id_for(index)`, `index_for(house_id)`, and `display_name_for(house_id)` helpers.
