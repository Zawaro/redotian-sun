...
The `bridge_kind` enum on `EntityData` carries low/high/rail; deck grade/rise and level are authored data, not hardcoded. Alternative rejected: a single bridge kind with runtime height probing — the destructibility and end-geometry differences are structural, not incidental.
