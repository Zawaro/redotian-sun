...
The system SHALL provide an `EntityFactory.gd` autoload singleton that creates entities from EntityData resources. The factory SHALL instantiate a base `Entity.tscn` scene and add components dynamically based on data properties. EntityFactory SHALL create DockHostComponent (renamed from DockComponent), DockClientComponent, and RefineryComponent.
...
#### Scenario: Harvester gets DockClientComponent
- **WHEN** EntityData has `harvester = true`, `dock = "PROC"`
- **THEN** entity gets DockClientComponent with `can_dock_with = ["PROC"]`

#### Scenario: Refinery gets DockHostComponent and RefineryComponent
- **WHEN** EntityData has `dock_position != Vector3.ZERO`, `accepted_resource_categories = ["tiberium"]`
- **THEN** entity gets DockHostComponent and RefineryComponent
