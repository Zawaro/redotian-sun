...
- Add a configurable `jumpjet_target_height` field to the `Locomotor` resource (terrain height units, default `5.0` ≈ `5 × HEIGHT_STEP` world units), replacing the shared `GlobalRules.hover_height` as the jumpjet's flight altitude. Applied to `resources/locomotors/Jumpjet.tres`.
...
- **Hover on arrival**: completing a fly order leaves the jumpjet hovering at the target height (TS-style); only a walk order triggers descent/landing.
...
### New Capabilities
- `jumpjet-vertical-transitions`: configurable jumpjet flight altitude, vertical ascend/descend state machine, zone-retaining attack movement, hover-on-arrival, and airborne terrain-speed exemption.
...
- **Tests**: unit tests for target-height config, ascend/descend transitions, zone retention on attack, hover-on-arrival, descend-interrupt ascend, and XZ combat range.
