...
A dispatched projectile SHALL be spawned at the firing socket's world transform composed with its yaw and extended by `barrel_length` along the socket's forward axis, not at the unit center. A body-mounted weapon SHALL spawn from the entity transform, extended by the weapon's barrel length.
