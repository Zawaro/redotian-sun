...
- **WHEN** player 0 owns a building whose `EntityData.storage_capacity` declares `{"tiberium": 2000}` and `EconomyManager.get_storage_capacity(0, "tiberium")` is called
- **THEN** it returns 2000

#### Scenario: Storage capacity with no buildings
- **WHEN** player 0 owns no buildings and `EconomyManager.get_storage_capacity(0, "tiberium")` is called
- **THEN** it returns 0
