...
- **WHEN** a map is exported
- **THEN** the JSON includes `"map_size": [grid_cells.x, grid_cells.y]` and `"visible_bounds_size": [grid_cells.x - 2*visible_offset_x, grid_cells.y - 2*visible_offset_z]`

#### Scenario: Backward compatibility with v3
- **WHEN** MapLoader reads a file with `"version": 3` and no `map_size` / `visible_bounds_size`
...
`TerrainSystem.export_to_json` SHALL write the full grid extent as `map_size` and the full visible play-area extent as `visible_bounds_size`, and `MapLoader` SHALL restore the visible bounds onto `BoundsSystem` when loading a map.

#### Scenario: Round-trip custom visible bounds
- **WHEN** a map with grid `40×40` and visible-bounds insets `(5, 4)` is exported and then loaded
- **THEN** the JSON `visible_bounds_size` is `[30, 32]` and after load `BoundsSystem.visible_offset_x == 5` and `visible_offset_z == 4`

#### Scenario: Bounds applied after terrain import
- **WHEN** MapLoader loads a v4 map
- **THEN** it imports terrain first (syncing `BoundsSystem.grid_cells`) and then applies `visible_bounds_size`, triggering a bounds redraw

#### Scenario: v3 fallback to default insets
- **WHEN** MapLoader loads a file with no `visible_bounds_size`
- **THEN** `BoundsSystem` visible-bounds insets are set to the default `(10, 8)`

#### Scenario: Clamp negative insets
- **WHEN** a hand-authored `visible_bounds_size` is larger than `map_size`
