...
#### Scenario: Mask baked only on state change
...
- **THEN** the RG8 edge mask is rebuilt alongside the state texture; when the buffer is unchanged, no rebake occurs
