...
- [ ] 1.3 Add setter to `grid_cells` in `TerrainSystem.gd` that calls `_init_vertex_grid()` with value clamping
...
- [x] 2.5 Remove duplicate `raise_cell()` call in `TestMap02.gd` (L19 or L20)
...
- [ ] 3.4 Move MapEditor UI construction from code to `scenes/editor/EditorUI.tscn`, reduce `_setup_ui()` to scene instantiation
- [ ] 3.5 Extract health bar and outline mesh generation from `SelectComponent.gd` into new `scripts/components/SelectionVisuals.gd` child node
