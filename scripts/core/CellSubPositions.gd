...
static func get_sub_positions(cell: Vector2i) -> Array[Vector3]:
    ...
    for i in range(NUM_SLOTS):
        var angle: float = base_angle + i * (TAU / NUM_SLOTS)
...
static func get_sub_position(cell: Vector2i, slot: int) -> Vector3:
    var positions := get_sub_positions(cell)
    return CellUtil.cell_to_world(cell) + positions[clampi(slot, 0, NUM_SLOTS - 1)]
