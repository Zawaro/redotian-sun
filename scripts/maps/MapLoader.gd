...
## House id for a map entity entry: an explicit `house_id` wins; legacy entries
## fall back to a `player_id` below the house-list size mapped onto that house.
## Returns "" when the entry carries no house information.
