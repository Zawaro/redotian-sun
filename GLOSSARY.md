...
| `bridge` land type | Runtime-derived surface at a deck level for cells covered by an intact bridge overlay; resolves only at its own level (`get_land_type(cell, level)`), is costed as road with the terrain figure skipped, and is never painted. The underlying ground land at level 0 is preserved and reverts when the piece leaves. | [bridges](openspec/changes/bridges-walkable-surface/specs/bridges/spec.md) · scripts/core/TerrainSystem.gd |
...
| rail bridge | A high-bridge variant only, never low; its end carries an equivalent rail road cut, and rail-specific movement restrictions belong to the movement system. | [bridges](openspec/changes/bridges-walkable-surface/specs/bridges/spec.md) · games/ts/entities/overlay/rail_bridge.tres |
...
| road cut | Three-cell road-grade cut through a high-bridge end `TerrainObject` (cliff), giving ground traffic a matching-grade path onto the deck; applied by the stamp-to-grid consumer and persisted as a `cell pin`. | [bridges](openspec/changes/bridges-walkable-surface/specs/bridges/spec.md) · games/ts/terrain_objects/cliff_bridge_end_n.tres |
| bridge piece / span | Authoring unit of a bridge: a row of three cells sharing one piece id; each covered cell is its own overlay entity at the piece's deck level. | [bridges](openspec/changes/bridges-walkable-surface/specs/bridges/spec.md) |
