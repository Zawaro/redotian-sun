...
# Credit counter tick SFX tests — the credit counter steps toward its
...
#
# Tests drive the counter's _step_counter with synthetic deltas and run without
...
func _label(counter: Control) -> Label:
    return counter as Label
...
func test_ready_forces_display_silently():
    ...
    TestHelper.assert_true(_label(counter) != null, "CreditsLabel scene root is a Label")
    TestHelper.assert_eq(
        _label(counter).text, "$%d" % balance, "ready shows the current balance instantly"
    )
...
func test_gain_animates_with_up_ticks():
    ...
    var counter := _make_counter()
    _label(counter).call("_force_display_credits", 500)
    ...
    TestHelper.assert_eq(_label(counter).text, "$500", "credit change does not jump the label")

    # Known example: gap 500, first step = floor(500 / 8) = 62.
    _arm_retrigger()
    var before := _count_audio_players()
    _label(counter).call("_step_counter", 0.0)
    TestHelper.assert_eq(
        _label(counter).text, "$562", "first gain step advances by floor(500/8)=62"
    )
    ...
    var calls := 0
    while _label(counter).text != "$1000" and calls < 100:
        _arm_retrigger()
        _label(counter).call("_step_counter", 0.0)
        calls += 1
    TestHelper.assert_eq(_label(counter).text, "$1000", "counter settles exactly at the target")
    ...
    _label(counter).call("_step_counter", 0.1)
...
func test_small_gain_uses_min_step():
    ...
    _install_econ_fixtures()
    var counter := _make_counter()
    _label(counter).call("_force_display_credits", 1000)
    ...
    for expected in [1001, 1002, 1003]:
        ...
        var before := _count_audio_players()
        _label(counter).call("_step_counter", 0.0)
        TestHelper.assert_eq(_label(counter).text, "$%d" % expected, "small gain steps by MIN_STEP")
    ...
    var before := _count_audio_players()
    _label(counter).call("_step_counter", 0.0)
    TestHelper.assert_eq(_label(counter).text, "$1003", "small gain settles exactly at target")
...
func test_spend_counts_down_slower():
    ...
    _arm_retrigger()
    var before := _count_audio_players()
    _label(counter).call("_step_counter", 0.04)
    TestHelper.assert_eq(_label(counter).text, "$1000", "spend does not step below its interval")
    ...
    _arm_retrigger()
    before = _count_audio_players()
    _label(counter).call("_step_counter", 0.05)
    TestHelper.assert_eq(
        _label(counter).text, "$938", "first spend step advances by floor(500/8)=62"
    )
...
func test_other_player_events_stay_silent():
    ...
    for i in range(10):
        _arm_retrigger()
        _label(counter).call("_step_counter", 1.0)
    TestHelper.assert_eq(
        _label(counter).text, "$1000", "other players' credit changes never animate the counter"
    )
...
func test_animation_completes_with_missing_audio_file():
    ...
    while _label(counter).text != "$1000" and calls < 100:
        ...
        calls += 1
    TestHelper.assert_eq(
        _label(counter).text, "$1000", "counter settles even when the tick id is missing"
    )
    TestHelper.assert_eq(
...
func test_counter_tracks_balance_during_gradual_production():
    ...
    _label(counter).call("_force_display_credits", 5000)
    ...
    # gradually (rate = cost / build_time = 120 credits/s here) and the credit
    # counter gets its delta — mirroring the two _process callbacks in-game.
    for i in range(120):
        pm._process(1.0 / 60.0)
        _label(counter).call("_step_counter", 1.0 / 60.0)
    ...
    (
        TestHelper
        . assert_true(
            _label(counter).text.trim_prefix("$").to_int() < 5000,
    ...
    var calls := 0
    while _label(counter).text != "$%d" % balance and calls < 200:
        _label(counter).call("_step_counter", 0.05)
        calls += 1
    TestHelper.assert_eq(
        _label(counter).text, "$%d" % balance, "counter settles at the drained balance"
    )
...
func test_spend_does_not_burst_after_sustained_income():
    ...
    TestHelper.assert_eq(_label(counter).text, "$5290", "in-flight gain counts at the MAX_STEP cap")
    ...
    var calls := 0
    while _label(counter).text != "$5280" and calls < 100:
        _label(counter).call("_step_counter", 0.05)
        calls += 1
    TestHelper.assert_eq(_label(counter).text, "$5280", "counter settles at the spend target")
    _drop_counter(counter)
